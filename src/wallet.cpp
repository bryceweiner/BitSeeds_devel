    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    printf("WalletUpdateSpent found spent coin %s BITS %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    printf("ReacceptWalletTransactions found spent coin %s BITS %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
...
{
	if (fDebug) printf("Selecting coins...Target value: %d\n",nTargetValue);
    vector<COutput> vCoins;
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    // Choose coins to use
    int64_t nBalance = GetBalance();
    if (fDebug) printf("CreateCoinStake: nBalance <= nResereveBalance : %d <= %d\n",nBalance,nReserveBalance);
    if (nBalance <= nReserveBalance)
        return false;
    ...
    {
    	if (fDebug) printf("CreateCoinStake: no suitable coins found!\n");
        return false;
    }

    if (setCoins.empty())
    {
    	if (fDebug) printf("CreateCoinStake: setCoins is empty!\n");
        return false;
    }
    ...
    CTxDB txdb("r");
    if (fDebug) printf("CreateCoinStake: Creating Coin Stake...\n");
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
    	if (fDebug) printf("CreateCoinStake: setting txindex...\n");
        CTxIndex txindex;
        {
            LOCK2(cs_main, cs_wallet);
            if (!txdb.ReadTxIndex(pcoin.first->GetHash(), txindex))
    	...
    	if (fDebug) printf("CreateCoinStake: reading block header...\n");
        // Read block header
        CBlock block;
        {
            LOCK2(cs_main, cs_wallet);
    	...
    	if (fDebug) printf("CreateCoinStake: matching coin age requirement...\n");
        static int nMaxStakeSearchInterval = 60;
        if (block.GetBlockTime() + nStakeMinAge > txNew.nTime - nMaxStakeSearchInterval)
            continue; // only count coins meeting min age requirement

	    if (fDebug) printf("CreateCoinStake: creating kernel...\n");
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                printf("FixSpentCoins found lost coin %s BITS %s[%d], %s\n",
            ...
            {
                printf("FixSpentCoins found spent coin %s BITS %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue).c_str(), pcoin->GetHash().ToString().c_str(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
